...
class GraphNV():
        ...
        for dst_nbr in sorted(G.neighbors(dst)):
            if dst_nbr == src:
                unnormalized_probs.append(
                    G[dst][dst_nbr][0][self.weight_key] / p)
            elif G.has_edge(dst_nbr, src):
                unnormalized_probs.append(
                    G[dst][dst_nbr][0][self.weight_key])
            else:
                unnormalized_probs.append(
                    G[dst][dst_nbr][0][self.weight_key] / q)
    ...
    def preprocess_transition_probs(self):
        ...
        alias_nodes = {}
        for node in G.nodes():
            unnormalized_probs = [G[node][nbr][0][self.weight_key]
